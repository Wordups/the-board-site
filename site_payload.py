...
def _build_fallback_hr_top10(results: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    Pulls active rosters from MLB Stats API for each team in today's games.
    Ranks by homeRuns (primary) and slugging (secondary).
    """
    import requests
        ...
        for team_id, team_abbr in [(home_id, home_abbr), (away_id, away_abbr)]:
            ...
            try:
                ...
                for entry in roster:
                    ...
                    # Skip pitchers
                    if position in ["P", "RP", "SP", "LP"]:
                        continue
                    
                    hitters.append({
                        "player": person.get("fullName", "Unknown"),
                        "team": team_abbr,
                        "homeRuns": _safe_int(entry.get("seasonStats", {}).get("hitting", {}).get("homeRuns") or 0),
                        "slugging": _safe_float(entry.get("seasonStats", {}).get("hitting", {}).get("sluggingPct") or 0),
                        "last10": "--",
                        "last5": "--",
                        "last3": "--",
                    })
            except Exception as e:
                print(f"[WARN] Fallback roster fetch failed for team {team_id}: {e}")
                continue
    
    # Sort by homeRuns desc, then slugging desc
